...
```{r}
library(gridExtra)
marrangeGrob(pearson_heatmap, nrow = 2, ncol = 2)
```
...
```{r}
marrangeGrob(spearman_heatmap, nrow = 2, ncol = 2)
```
...
```{r}
marrangeGrob(kendall_heatmap, nrow = 2, ncol = 2)
```
...
diff_dist_vec <- sort(diff_dist, decreasing = T)

for (i in 1:4){
  diff_dist_ind <- which(diff_dist == diff_dist_vec[i],
                            arr.ind = T)
...
```{r}
marrangeGrob(dist_heatmap, nrow = 2, ncol = 2)
```
...
diff_hoeff_vec <- sort(diff_hoeff, decreasing = T)

for (i in 1:4){
  diff_hoeff_ind <- which(diff_hoeff == diff_hoeff_vec[i],
                            arr.ind = T)
  ...
  plot(sub_full[, diff_idx1], sub_full[, diff_idx2], asp = T, pch = 16,
       ...
       main = paste0("Correlation of ",
                     round(hoeffd(sub_full[, diff_idx1],
                               sub_full[, diff_idx2])$D[1,2], 3),
                            "\nDifference of ",
                     round(diff_hoeff_vec[i], 3)))
  for (j in 1:length(cell_labels)){
    sub_dat <- subset_data[[j]]
    plot(sub_dat[,diff_idx1], sub_dat[,diff_idx2], asp = T,
         pch = 16, col = cell_labels[j],
         ...
         ylab = paste0(colnames(sub_dat)[diff_idx2], ", (", diff_idx2, ")"), 
         main = paste0("Correlation of ",
                       round(hoeff_array[diff_idx1, diff_idx2, j], 3),
                       "\nCell_type: ", cell_labels[j]),
         xlim = c(-1, 3), ylim = c(-2, 4))
  }
}
...
```{r}
marrangeGrob(hoeff_heatmap, nrow = 2, ncol = 2)
```
...
diff_MI <- apply(MI_array, MARGIN = c(1,2),
                      FUN = function(x) {diff(range(x))})
diff_MI_vec <- sort(diff_MI, decreasing = T)


for (i in 1:4){
  diff_MI_ind <- which(diff_MI == diff_MI_vec[i],
                       arr.ind = T)
  ...
  for (j in 1:length(cell_labels)){
    ...
    plot(sub_dat[,diff_idx1], sub_dat[,diff_idx2], asp = T, pch = 16,
         ...
         ylab = paste0(colnames(sub_dat)[diff_idx2], ", (", diff_idx2, ")"), 
         main = paste0("Correlation of ",
                       round(MI_array[diff_idx1, diff_idx2, j], 3),
                       "\nCell_type: ", cell_labels[j]),
...
```{r}
marrangeGrob(MI_heatmap, nrow = 2, ncol = 2)
```
...
diff_MIC <- apply(MIC_array, MARGIN = c(1,2),
                      FUN = function(x) {diff(range(x))})
diff_MIC_vec <- sort(diff_MIC, decreasing = T)

for (i in 1:4){
  diff_MIC_ind <- which(diff_MIC == diff_MIC_vec[i],
                        arr.ind = T)
  ...
  for (j in 1:length(cell_labels)){
    ...
    plot(sub_dat[,diff_idx1], sub_dat[,diff_idx2], asp = T,
        ...
        ylab = paste0(colnames(sub_dat)[diff_idx2], ", (", diff_idx2, ")"), 
        main = paste0("Correlation of ",
                      round(MIC_array[diff_idx1, diff_idx2, j], 3),
                      "\nCell_type: ", cell_labels[j]),
...
```{r}
marrangeGrob(MIC_heatmap, nrow = 2, ncol = 2)
```
...
diff_XI <- apply(XI_array, MARGIN = c(1,2),
                      FUN = function(x) {diff(range(x))})
diff_XI_vec <- sort(diff_XI, decreasing = T)


for (i in 1:4){
  diff_XI_ind <- which(diff_XI == diff_XI_vec[i],
                       arr.ind = T)
  ...
  for (j in 1:length(cell_labels)){
    ...
    plot(sub_dat[,diff_idx1], sub_dat[,diff_idx2], asp = T, 
        ...
        ylab = paste0(colnames(sub_dat)[diff_idx2], ", (", diff_idx2, ")"), 
        main = paste0("Correlation of ",
                      round(XI_array[diff_idx1, diff_idx2, j], 3),
                      "\nCell_type: ", cell_labels[j]),
        xlim = c(-1, 4), ylim = c(-2, 4))
  }
}
```
...
```{r}
marrangeGrob(XI_heatmap, nrow = 2, ncol = 2)
```
...
diff_HSIC <- apply(HSIC_array, MARGIN = c(1,2),
                      FUN = function(x) {diff(range(x))})
diff_HSIC_vec <- sort(diff_HSIC, decreasing = T)

for (i in 1:4){
  diff_HSIC_ind <- which(diff_HSIC == diff_HSIC_vec[i],
                         arr.ind = T)
  ...
  for (j in 1:length(cell_labels)){
    ...
    plot(sub_dat[,diff_idx1], sub_dat[,diff_idx2], asp = T,
        ...
        ylab = paste0(colnames(sub_dat)[diff_idx2], ", (", diff_idx2, ")"), 
        main = paste0("Correlation of ",
                      round(HSIC_array[diff_idx1, diff_idx2, j], 3),
                      "\nCell_type: ", cell_labels[j]),
...
```{r}
marrangeGrob(HSIC_heatmap, nrow = 2, ncol = 2)
```
...
diff_Blomqvist <- apply(Blomqvist_array, MARGIN = c(1,2),
                      FUN = function(x) {diff(range(x))})
diff_Blomqvist_vec <- sort(diff_Blomqvist, 
                           decreasing = T)

for (i in 1:4){
  diff_Blomqvist_ind <- which(diff_Blomqvist == diff_Blomqvist_vec[i],
                              arr.ind = T)
  
  diff_idx1 <- diff_Blomqvist_ind[i, 1]; diff_idx2 <- diff_Blomqvist_ind[i,2]
  ...
  for (j in 1:length(cell_labels)){
    ...
    plot(sub_dat[,diff_idx1], sub_dat[,diff_idx2], asp = T,
        ...
        ylab = paste0(colnames(sub_dat)[diff_idx2], ", (", diff_idx2, ")"), 
        main = paste0("Correlation of ",
                      round(Blomqvist_array[diff_idx1, diff_idx2, j], 3),
                      "\nCell_type: ", cell_labels[j]),
...
```{r}
marrangeGrob(Blomqvist_heatmap, nrow = 2, ncol = 2)
```
