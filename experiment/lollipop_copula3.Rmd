...
# one
diag(cov_mat_first) <- c(0.9, 1.1, 1, 0.9, 1.1,
                         1.1, 0.9, 0.9, 1, 1)
...
- Explore with changing the variance of second and third cell types to higher values. (For academic purpose!!) In contrast, close to 1 for first cell type. We are going to explore this change and then come back (basically study from both sides)
- Try to make a few of them that look like the lollipops in the "Lollipop_Explore2" file. (among 10 choose 2 plots!)
- After getting a diverse contrast pattern, we can jump into new cell type data.
...
# higher variance up to 10
diag(cov_mat_third) <- sample(c(1,2,25,26), 10, replace = TRUE)
...
## 3-2-1. High Pearson (> 0.80) and Low Spearman (< 0.20) (linearity vs monotone)
...
## 3-2-2. Visualization of High Pearson (> 0.80) and Low Spearman (< 0.20) (linearity vs monotone)
...
## 3-4-1. High Pearson (> 0.80) and Low Kendall (< 0.15) (linearity vs monotone)

```{r}
cor_contrast4 <- (abs(cor_pearson_mat) > 0.80) & (abs(cor_kendall_mat) < 0.15)
cor_contrast_ind4 <- which(cor_contrast4, arr.ind = T)
nrow(cor_contrast_ind4)
```

## 3-4-2. Visualization of High Pearson (> 0.80) and Low Kendall (< 0.15) (linearity vs monotone)
...
## 3-6-1. High Pearson (> 0.85) and Low Hoeffding's D (< 0.15) (linearity vs monotone)

```{r}
cor_contrast6 <- (abs(cor_pearson_mat) > 0.85) & (abs(cor_hoeffd_mat) < 0.15)
cor_contrast_ind6 <- which(cor_contrast6, arr.ind = T)
nrow(cor_contrast_ind6)
```

## 3-6-2. Visualization of High Pearson (> 0.85) and Low Hoeffding's D (< 0.15) (linearity vs monotone)
...
## 3-8-1. High Pearson (> 0.50) and Low Blomqvist's beta (< 0.50) (linearity vs monotone)

```{r}
cor_contrast8 <- (abs(cor_pearson_mat) > 0.5) & (abs(cor_blomqvist_mat) < 0.5)
cor_contrast_ind8 <- which(cor_contrast8, arr.ind = T)
nrow(cor_contrast_ind8)
```

## 3-8-2. Visualization of High Pearson (> 0.50) and Low Blomqvist's beta (< 0.50) (linearity vs monotone)
...
## 3-10-1. High Pearson (> 0.85) and Low XI (< 0.2) (linearity vs non-linearity)

```{r}
cor_contrast10 <- (abs(cor_pearson_mat) > 0.85) & (abs(cor_XI_mat) < 0.2)
cor_contrast_ind10 <- which(cor_contrast10, arr.ind = T)
nrow(cor_contrast_ind10)
```

## 3-10-2. Visualization of High Pearson (> 0.85) and Low XI (< 0.2) (linearity vs non-linearity)
...
## 3-11-1. Low Spearman (< 0.20) and High Distance Correlation (> 0.85) (monotone vs non-linearity)

```{r}
cor_contrast11 <- (abs(cor_spearman_mat) < 0.2) & (abs(cor_dist_mat) > 0.85)
cor_contrast_ind11 <- which(cor_contrast11, arr.ind = T)
nrow(cor_contrast_ind11)
```

## 3-11-2. Visualization of Low Spearman (< 0.20) and High Distance Correlation (> 0.85) (monotone vs non-linearity)
...
## 3-12-1. High Spearman (> 0.50) and Low Distance Correlation (< 0.50) (monotone vs non-linearity)

```{r}
cor_contrast12 <- (abs(cor_spearman_mat) > 0.5) & (abs(cor_dist_mat) < 0.5)
cor_contrast_ind12 <- which(cor_contrast12, arr.ind = T)
nrow(cor_contrast_ind12)
```

## 3-13-1. Low Kendall (< 0.15) and High Distance Correlation (> 0.85) (monotone vs non-linearity)

```{r}
cor_contrast13 <- (abs(cor_kendall_mat) < 0.15) & (abs(cor_dist_mat) > 0.85)
cor_contrast_ind13 <- which(cor_contrast13, arr.ind = T)
nrow(cor_contrast_ind13)
```

## 3-13-2. Visualization of Low Kendall (< 0.15) and High Distance Correlation (> 0.85) (monotone vs non-linearity)
...
## 3-16-1. High Distance Correlation (> 0.85) and Low Hoeffiding's D (< 0.15) (non-linearity vs monotone)

```{r}
cor_contrast16 <- (abs(cor_dist_mat) > 0.85) & (abs(cor_hoeffd_mat) < 0.15)
cor_contrast_ind16 <- which(cor_contrast16, arr.ind = T)
nrow(cor_contrast_ind16)
```

## 3-16-2. Visualization of High Distance Correlation (> 0.85) and Low Hoeffiding's D (< 0.15) (non-linearity vs monotone)
...
## 3-18-1. High Distance Correlation (> 0.85) and Low XI (< 0.15) (non-linearity vs non-linearity)

```{r}
cor_contrast18 <- (abs(cor_dist_mat) > 0.85) & (abs(cor_XI_mat) < 0.15)
cor_contrast_ind18 <- which(cor_contrast18, arr.ind = T)
nrow(cor_contrast_ind18)
```

## 3-18-2. Visualization of High Distance Correlation (> 0.85) and Low XI (< 0.15) (non-linearity vs non-linearity)
...
## 3-20-1. High Distance Correlation (> 0.50) and Low Blomqvist's Beta (< 0.50) (non-linearity vs non-linearity)

```{r}
cor_contrast20 <- (abs(cor_dist_mat) > 0.5) & (abs(cor_blomqvist_mat) < 0.5)
cor_contrast_ind20 <- which(cor_contrast20, arr.ind = T)
nrow(cor_contrast_ind20)
```

## 3-20-2. Visualization of High Distance Correlation (> 0.50) and Low Blomqvist's Beta (< 0.50) (non-linearity vs non-linearity)
