...
- Define a dynamic function that finds the highest 6, given 

```{r}
head(bm[["pca"]]@cell.embeddings)


quantile_mat <- c()
for (i in 1:length(cormat_list)){
  quantile_mat <- rbind(quantile_mat,
                        quantile(abs(cormat_list[[i]]), probs = c(0.20, 0.85), na.rm = T))
}
...
```{r}
new_dat <- (bm[["pca"]]@cell.embeddings) %*% t((bm[["pca"]]@feature.loadings))

index <- sample(1:prod(dim(new_dat)), 5000)

full_dat <- t(bm@assays$RNA@scale.data)

plot(x = new_dat[index], y = full_dat[index], asp = T)
